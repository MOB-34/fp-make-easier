...
instance functorRWS :: Functor (RWS r w s) where
  map f (RWS fa) = RWS \rws -> fa rws # \(Tuple x rws') -> Tuple (f x) rws'
...
instance bindRWS :: Monoid w => Bind (RWS r w s) where
  bind (RWS f) mf = RWS \rws -> (f rws)
    # \(Tuple a rws'@{ w: w1 }) -> runRWS (mf a) rws'
        # \(Tuple a' rws''@{ w: w2 }) -> Tuple a' rws'' { w = w1 <> w2 }
...
rwsTest = do
  tell [ "test the log" ]
  tell [ "test the log2", "test the log2" ]
  config <- ask
